...
const RentBook = () => {
  ...
  const handleRentDateChange = (date: Date) => {
    if (date.toLocaleDateString('en-CA') === rentDate?.toLocaleDateString('en-CA')) {
      // If the same date is clicked, disable the filter
      setRentDate(date);
      setIsInitialRender(true); // Reset to initial render state
    } else {
      // Otherwise, set the new date and apply the filter
      setRentDate(date);
      setIsInitialRender(false);
      refetch(); // Trigger API query
    }
  };
      ...
      <div className="d-flex justify-content-between align-items-end">
        ...
        <div className="d-flex gap-5">
          <div className="col-12">
            <SimpleDatePicker label="Rent Date" onDateChange={handleRentDateChange} />
          </div>
        </div>
